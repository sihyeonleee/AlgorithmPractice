 ...
 * 같은 값이 연속해서 나타나는 것을 그 문자의 개수와 반복되는 값으로 표현하여 더 짧은 문자열로 
 * 줄여서 표현하는 알고리즘을 공부하고 있습니다.
 * 간단한 예로 "aabbaccc"의 경우 "2a2ba3c"(문자가 반복되지 않아 한번만 나타난 경우 1은 생략함)와 
 * 같이 표현할 수 있는데, 이러한 방식은 반복되는 문자가 적은 경우 압축률이 낮다는 단점이 있습니다. 
 * 예를 들면, "abcabcdede"와 같은 문자열은 전혀 압축되지 않습니다. "어피치"는 이러한 단점을 해결하기 
...
/**
 * Code
 * 주어진 문제와 다르게 어느 위치에서는 반복 문장을 압축하는 코드
 */
function solution(s) {
    let minLength = s.length;
    let range = Math.floor(minLength / 2);
    for(let r=1; r<=range; r++){
        let length = start(s, r).length;
...
function start(s, r){
    ...
    let result = '';
    let length = s.length;
    let minimumLength = r * 2;
    while(length - index >= minimumLength){
        let str = s.substr(index, r);
        let cnt = find(s, index+r, r, str) + 1;
        if(cnt == 1){
            result += s[index++];
        }else {
            result += str+cnt;
    ...
    return result += s.substr(index);
    
}

function find(s, i, r, f){
    let cnt = 0;
    let t = s.substr(i, r);
    if(t == f) {
        cnt += find(s, i+r, r, t) + 1;
    } 
    return cnt;
}
